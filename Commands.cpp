	...
	for (int i = 0; i < (int) target.size(); ++i)
	{
		...
		else if (type[i] == 1)
		{
			...
			if (target_client != NULL)
			{
				...
				s << ":" << create_tag(sender) << " PRIVMSG " << target_client->get_nickname() << " :" << msg << std::endl ;
...
void Server::join(Client& sender, std::string chan_name, std::string key)
{
	...
	if (chan != NULL)
	{
		...
		{
			std::cout << "Error :" << sender.get_nickname() << " not invited" << std::endl;
			send_err(473, sender, chan_name, "Cannot join channel (invite-only)");
			return ;
		}
		if (chan->is_mode('l') && chan->count_membres() >= chan->get_limit())
		{
			std::cout << "Error :" << sender.get_nickname() << " users limit reached" << std::endl;
		...
		if (chan->is_mode('k'))
		{
			if (key.empty())
			{
				std::cout << "Error :" << sender.get_nickname() << " key not supplied" << std::endl;
				send_err(475, sender, chan_name, "Cannot join channel (key not supplied)");
				return ;
			}
			else if (chan->get_key() != key)
			{
				std::cout << "Error :" << sender.get_nickname() << " incorrect key" << std::endl;
...
void 	Server::part(Client &sender, std::string chan_name, std::string msg)
{
	...
	if (chan != NULL)
	{
		...
		{
			std::cout << "Error : " << chan_name << " You're not on that channel" << std::endl;
...
// · l: Set/remove the user limit to channel
	...
	else
	{
		...
		if (target_client != NULL)
		{
			...
			{
				std::cout << "Error : " << target << " Cant change mode for other users" << std::endl;
	...
	if (target.at(0) == '#')
	{
		...
		if (chan != NULL)
		{
			if (chan->is_mode('t'))
			{
				...
				{
					std::cout << "Error : #" << target << " You're not channel operator" << std::endl;
